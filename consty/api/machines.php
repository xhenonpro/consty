...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        $stmt = $pdo->query('SELECT id, name, quantity FROM machines ORDER BY id DESC');
        ...
        $stmt = $pdo->prepare('INSERT INTO machines (name, quantity) VALUES (?, ?)');
        ...
        $params[] = $id;
        $stmt = $pdo->prepare('UPDATE machines SET ' . implode(', ', $fields) . ' WHERE id = ?');
        $stmt->execute($params);
        echo json_encode(['success' => true]);
    }
} catch (Exception $e) {
