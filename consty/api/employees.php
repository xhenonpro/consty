...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        $stmt = $pdo->query('SELECT id, name, salary, project_id, email, phone FROM employees ORDER BY id DESC');
        ...
        if (isset($input['salary'])) { $fields[] = 'salary = ?'; $params[] = $input['salary']; }
        ...
        if (!$id || count($fields) === 0) {
            http_response_code(400);
            echo json_encode(['error' => 'Employee id and at least one field are required']);
            exit();
        }
        $params[] = $id;
