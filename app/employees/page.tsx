    ...
    <RequireAuth>
      <div className="w-full max-w-5xl mx-auto py-8 px-2 md:px-0">
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
            <thead className="bg-blue-50 dark:bg-blue-950">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-bold text-blue-700 dark:text-blue-300 uppercase tracking-wider">Salary</th>
              ...
              {loading ? (
                ...
                employees.map((e) => (
                  <tr key={e.id} className="hover:bg-blue-50 dark:hover:bg-blue-950 transition">
                    <td className="px-6 py-4 font-semibold text-gray-900 dark:text-gray-100">{e.name}</td>
                    <td className="px-6 py-4">${Number(e.salary).toFixed(2)}</td>
                    <td className="px-6 py-4">{e.project_id || <span className="text-gray-400">-</span>}</td>
                    <td className="px-6 py-4">{e.email || <span className="text-gray-400">-</span>}</td>
                    <td className="px-6 py-4">{e.phone || <span className="text-gray-400">-</span>}</td>
        ...
        <Suspense fallback={null}>
          {showModal && <AddEditEmployeeModal onClose={() => { setShowModal(false); setEditEmployee(null); }} onSave={handleAddEdit} employee={editEmployee} />}
        </Suspense>
        <style jsx global>{`
          @keyframes fadeIn {
            from { opacity: 0; }
